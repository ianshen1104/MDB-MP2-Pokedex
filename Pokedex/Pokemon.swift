...
class Pokemon: Decodable {
    ...
    */
    ...
    required init(from decoder: Decoder) throws {
        ...
        self.normalImageUrl = try imageContainer.decode(String.self, forKey: .normal)
        self.largeImageUrl = try imageContainer.decode(String.self, forKey: .large)
        self.gifImageUrl = try imageContainer.decode(String.self, forKey: .animated)
    }
    
    func getSmallImage() -> UIImage {
        guard let url = URL(string: normalImageUrl) else {
            return UIImage(imageLiteralResourceName: "default-image")
        }
    ...
    func getGifImage() -> UIImage {
        guard let gitImage = UIImage.gifImageWithURL(gifImageUrl) else {
            return UIImage(imageLiteralResourceName: "default-image")
        }
        /*
        guard let data = try? Data(contentsOf: url) else {
